...
load sim_section_size_increase_large_dataset_net_age_12_adv_0.1.dat
...
load sim_section_size_increase_large_dataset_net_age_12_adv_0.2.dat
jj = 2;
first_iteration_above_threshold(jj,:) = network_iterations*ones(size(number_of_sections));
for ii = 1:length(number_of_sections)
    first_above_threshold = min(find(stats{ii}.malicious_elders_fraction_max > 1/3));
...
load sim_section_size_increase_large_dataset_net_age_12_adv_0.25.dat
jj = 3;
first_iteration_above_threshold(jj,:) = network_iterations*ones(size(number_of_sections));
for ii = 1:length(number_of_sections)
    first_above_threshold = min(find(stats{ii}.malicious_elders_fraction_max > 1/3));
...
load sim_section_size_increase_large_dataset_net_age_12_adv_0.3.dat
jj = 4;
first_iteration_above_threshold(jj,:) = network_iterations*ones(size(number_of_sections));
for ii = 1:length(number_of_sections)
    first_above_threshold = min(find(stats{ii}.malicious_elders_fraction_max > 1/3));
